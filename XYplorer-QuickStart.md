...
## 多个标签栏（New Tab）快捷打开&关闭

![New Tab On&Off.gif](_resources/NewTabOn&Off.gif)

鼠标双击关闭录制GIF忘了录进去，到时候修改配置后你们也可以自己尝试

## 快速预览(Hover Box)

方法一：ALT按住，同时鼠标移动到图片上就行了（活动窗口下使用）

![Show Hover Box.gif](_resources/ShowHoverBox.gif)

方法二：F9->进入配置->显示悬停框->应用到...->需要预览的文件格式
...
没错，居然按CTRL+B（字体加粗），这样就收藏了，切换时就在导航面板上鼠标右键->选择文件夹。如果管理收藏夹：在工具栏中，收藏夹->管理收藏夹文件夹。

使用存在的问题：
比如，我的看图文件是Honeyview.exe,然而我关联我图像格式后，打开还是弹出类似微软的提示选择格式框，这该怎么办呢？
...
*路径之间按语法格式修改:* 文件格式>文件路径
获取文件路径的小技巧：对着应用软件ICON图标按住【SHIFT+鼠标右键】就可以看到:

![Tip To Get File Path.png](_resources/TipToGetFilePath.png)

于是出现复制的内容："C:\Users\vip57\Desktop\搜索 Everything.lnk"。添加文件路径时，把文件路径首尾的双引号”去掉。

**这样做的好处：** 

比如你安装了Potplayer，又安装了迅雷影音,结果迅雷影音把视频文件格式设置用迅雷影音打开，那么你用原生的文件管理器肯定双击视频文件后就是用迅雷影音打开。但是有了视频关联，就不会出现这种问题，只要你是用XYplorer就可以了。
再举个例子：
...
就是这样。不管Window设置中默认设置怎么变，这种Window默认设置是相对原生Explorer来说的，不会影响到XYplorer。
